...
func OnFileUploadFinished(filehash string, filename string, filesize int64, fileaddr string) bool {
	...
	if rf, err := ret.RowsAffected(); err == nil{
		if rf <= 0 {
			fmt.Printf("file with hash:%s has bean uploaded before", filehash)
		}
		return true
	}
	return false
...
func GetFileMeta(filehash string) (*TableFile, error) {
	stmt, err := mydb.DBConn().Prepare("select file_sha1, file_addr, file_name, file_size from tbl_file where file_sha1 = ?" +
